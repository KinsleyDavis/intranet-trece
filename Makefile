...
PROJECT_NAME=intranet-trece
STACK_NAME=intranet-tre-ce-jus-br
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://intranet-trece.localhost"
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t kinsleydavis/intranet-trece-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t kinsleydavis/intranet-trece-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name intranet-trece-backend-acceptance -d kinsleydavis/intranet-trece-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name intranet-trece-frontend-acceptance --link intranet-trece-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d kinsleydavis/intranet-trece-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop intranet-trece-frontend-acceptance
	@docker stop intranet-trece-backend-acceptance
